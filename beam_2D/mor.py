...
#			temp = mat*vec
		...
		self.construct_Jk(K)
		self.symplectic_proj()
		Jtk = self.A_plus * self.X * self.Jn * self.X * np.transpose(self.A_plus)
		self.T = self.compute_JJ_transformation(Jtk)

		print( np.max( np.abs( self.Jk - self.T*Jtk*np.transpose(self.T) ) ) )
