...
%	f = func(x,z0r,dt,Kr,bdr,Phi,PhiX)
	...
	Z = time_stepping(z0r,Kr,bdr,dt,MAX_ITER,K_energy,N,dx,Phi,PhiX,Psi)

function Z = time_stepping(z0,K,bd,dt,MAX_ITER,K_energy,N,dx,Phi,PhiX,Psi)
	z = z0;
	Z = z;

	for i=1:MAX_ITER
%		z = newton(z,z,dt,K,bd);
		z = newton(z,z,dt,K,bd,Phi,PhiX,Psi);
...
function f = func(x,x0,dt,L,bd,Phi,PhiX,Psi)
	x_full = Phi*x;
	x0_full = Phi*x0;
	N = size(x_full,1)/2;
	q = x_full(1:N);
	q0 = x0_full(1:N);
	nonlin = sin( (q+q0)/2 );
	nonlin = PhiX*Psi*[ zeros(N,1) ; nonlin ];
	f = (x-x0)/dt - L*(x+x0)/2 + nonlin - bd;

function Jf = jacob(x,x0,dt,L,Phi,PhiX,Psi)
	x_full = Phi*x;
	x0_full = Phi*x0;
	N = size(x_full,1)/2;
	q = x_full(1:N);
	q0 = x0_full(1:N);
	nonlin = cos( (q+q0)/2 )/2;
	nonlin = PhiX*Psi*[ zeros(N,2*N) ; diag(nonlin) , zeros(N,N) ]*Phi;

	K = size(x,1)/2;
	Jf = eye(2*K)/dt - L/2 + nonlin;

function x = newton(x0,z,dt,L,bd,Phi,PhiX,Psi)
	x = x0;
	for i=1:5
		f = func(x,z,dt,L,bd,Phi,PhiX,Psi);
		Jf = jacob(x,x0,dt,L,Phi,PhiX,Psi);
		x = x - inv(Jf)*f;
	end
