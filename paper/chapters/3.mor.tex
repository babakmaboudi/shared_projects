...
\right.
\end{equation}
Here $z\in \mathcal Z$ is the state vector, $\mathcal Z$ is a symplectic linear vector space with the canonical basis\todo{define canonical basis} $Z =\{ e_i,f_i \}_{i=1}^n$\todo{unused notation} \cite{Marsden:2010:IMS:1965128}, and $H:\mathbb R^{2n}\to\mathbb R$ is sufficiently smooth function. Suppose that the solution to (\ref{eq:mor.8}) is well approximated by a low dimensional symplectic subspace \cite{Marsden:2010:IMS:1965128}. Let $A\in \mathbb{R}^{2n\times 2k}$ be a $\mathbb{J}_{2n}$-symplectic basis containing the basis vectors $A=[\tilde e_1|\dots|\tilde e_k|\tilde f_1|\dots|\tilde f_k]$\todo{unused notation}, such that $z \approx Ay$ with $y \in \mathbb{R}^{2k}$ the expansion coefficients of $z$ in this basis. Using the symplectic inverse $A^+ := \mathbb J_{2k}^T A^T \mathbb J_{2n}$ we can construct the reduced system
...
\end{equation}
with the Hamiltonian $\mathcal H(y) = H(Ay)$\todo{unused notation}.
...
\end{equation}
Here, \editA{$\alpha = \{\alpha_i\}_{i=1}^{2k}$} are the expansion coefficients of the projection of $z$ onto colspan$(A_{2k})$ such that $\alpha_i = -\Omega(z_{k+1},\mathbb J_{2n}^Te_i)$\todo{should this be $+\Omega$ ?} for $i\leq k$ and $\alpha_i = \Omega(z_{k+1},e_i)$ for $i>k$. Since $\Omega(e_{k+1},\mathbb{J}_{2n}^T e_{k+1}) = \| e_{k+1} \|_2^2 \neq 0$ the enriched basis $A_{2k+2}$ reads
\begin{equation} \label{eq:mor.15}
	A_{2k+2} = [e_1|\dots|e_k|e_{k+1}|\mathbb J_{2n}^Te_1|\dots|\mathbb J_{2n}^Te_{k+1}].	
\end{equation}
It is easily verified that $A_{2k+2}$ is $\mathbb J_{2n}$-symplectic and orthonormal. This enrichment continues until the given tolerance is satisfied. We note that the choice of the orthogonalization routine generally depends on the application. In this paper we use the symplectic \editA{stable} Gram-Schmidt (GS) process as the orthogonalization routine. However the isotropic Arnoldi method \todo{this is the same as symplectic stable Gram-Schmidt} or the isotropic Lanczos method \cite{doi:10.1137/S1064827500366434} are backward stable alternatives.
