...
\editA{\begin{eqnarray} \label{eq:mor.9}
...
\begin{eqnarray} \label{eq:mor.10}
...
\begin{eqnarray} \label{eq:mor.11}
...
\begin{eqnarray} \label{eq:mor.12}
\left\{
\begin{aligned}
...
\begin{eqnarray} \label{eq:mor.13}
\begin{aligned}
...
\begin{eqnarray} \label{eq:mor.14}
...
\begin{eqnarray} \label{eq:mor.14.1}
...
\begin{eqnarray} \label{eq:mor.15}
...
\begin{eqnarray} \label{eq:mor.16}
...
\caption{The greedy algorithm for generation of a $\mathbb J_{2n}$-symplectic basis} 
\begin{tabular}{l l}
\multicolumn{2}{l}{{\bf Input:} Projection error tolerance $\delta$, initial condition $ z_0$,}\\
\multicolumn{2}{l}{snapshots $\mathcal Z = \{ z(t_i) \}_{i=1}^{N}$, and $\mathcal G = \{ \nabla_z H(z(t_i)) \}_{i=1}^{N}$} \\
1 & $e_1 \leftarrow \frac{z_0}{\|z_0\|_2}$ \\
2 & $A \leftarrow [e_1|\mathbb J^T_{2n}e_1]$
\\ 3 & $k \leftarrow 1$
\\ 4 & \textbf{while} $\| z - P^\text{symp}_{I,A}( z ) \|_2 > \delta$ for any $z\in \mathcal Z$
\\ 5 & \hspace{0.5cm} $z_{k+1} := \underset{z\in \mathcal Z}{\text{argmax }} \| z - P^\text{symp}_{I,A}( z ) \|_2$
\\ 6 & \hspace{0.5cm} $\mathbb J_{2n}$-orthogonalize $ z_{k+1}$ to obtain $e_{k+1}$
\\ 7 & \hspace{0.5cm} $A \leftarrow [e_1|\dots |e_{k+1} | \mathbb J^T_{2n}e_1|\dots,\mathbb J^T_{2n}e_{k+1}]$
\\ 8 & \hspace{0.5cm} $k \leftarrow k+1$
\\ 9 & \textbf{end while}
\\ 10 & compute $(A^+)^T=[e'_1|\dots|e'_k|\mathbb J^T_{2n}e'_1|\dots|\mathbb J^T_{2n}e'_k]$
\\ 11 & \textbf{while} $\| g - P^\text{symp}_{I,(A^+)^T}(g) \|_2 > \delta$ for all $g \in \mathcal G$
\\ 12 & \hspace{0.5cm} $g_{k+1} := \underset{g \in \mathcal G}{\text{argmax }} \| g - P^\text{symp}_{I,(A^+)^T}(g) \|_2$
\\ 13 & \hspace{0.5cm} $\mathbb J_{2n}$-orthogonalize $g_{k+1}$ to obtain $e'_{k+1}$
\\ 14 & \hspace{0.5cm} $(A^+)^T \leftarrow [e'_1|\dots |e'_{k+1} | \mathbb J^T_{2n}e'_1|\dots|\mathbb J^T_{2n}e'_{k+1}]$
\\ 15 & \hspace{0.5cm} $k \leftarrow k+1$
\\ 16 & \textbf{end while}
\\ 17 & $A \leftarrow \left( \left( \left( A^+\right) ^T \right ) ^+ \right)^T$
\\ \multicolumn{2}{l}{{\bf Output:} $\mathbb J_{2n}$-symplectic basis $A$.}
\end{tabular}
\label{alg:1}
\end{table}