...
It is worthwhile to note that the proper orthogonal decomposition (POD) method \cite{hesthaven2015certified} corresponds to taking $X = I_n$. In this case, the projection $P_{I_n,V}$ constructed from the solution to (\ref{eq:mor.5}) becomes the POD-Galerkin projection \cite{hesthaven2015certified}.
...
\end{equation}
This system is a Hamiltonian system with the Hamiltonian $\tilde H(y) = y^T A^T L A y + f(Ay)$. To reduced the complexity of evaluating the nonlinear term in (\ref{eq:mor.10}), we may apply the discrete empirical interpolation method (DEIM) \cite{Chaturantabut:2010cz}. Assuming that $\nabla_z f(z)$ lies on a low dimensional subspace with a basis $U$ the DEIM approximation reads
